...
const OrdersManager: React.FC<OrdersManagerProps> = ({ orders, customers, products, onUpdateOrder, onConfirmOrder }) => {
  ...
  return (
    <div className="space-y-6 pb-20">
        ...
        <div className="flex gap-1 overflow-x-auto no-scrollbar pb-2">
          {(['ALL', 'NEW', 'ACCEPTED', 'CONFIRMED', 'CANCELLED'] as const).map(f => (
              ...
              >
                ...
                {f === 'NEW' && newCount > 0 && (
                    <span className="absolute top-0 right-0 w-5 h-5 bg-red-500 text-white text-[8px] font-black flex items-center justify-center rounded-full animate-pulse z-10 transform translate-x-1 -translate-y-1 shadow-sm">
                      {newCount}
                    </span>
                )}
                {f === 'ACCEPTED' && acceptedCount > 0 && (
                    <span className="absolute top-0 right-0 w-5 h-5 bg-amber-500 text-white text-[8px] font-black flex items-center justify-center rounded-full z-10 transform translate-x-1 -translate-y-1 shadow-sm">
                      {acceptedCount}
                    </span>
                )}
              </button>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setSelectedOrder(null)}>
          <div className="bg-white w-full max-w-lg rounded-t-[40px] sm:rounded-[40px] shadow-2xl p-6 max-h-[95vh] flex flex-col animate-slide-up overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                ...
                {(selectedOrder.status === 'ACCEPTED' || selectedOrder.status === 'NEW') && (
                  <button onClick={() => setIsEditing(true)} className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center active:scale-90 transition-transform">
                    <i className="fas fa-pen text-sm"></i>
                  </button>
                )}
              </div>
              <button onClick={() => setSelectedOrder(null)} className="w-12 h-12 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center"><i className="fas fa-times text-xl"></i></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-5 no-scrollbar pb-6 pr-1">
              ...
              {/* Выбор способа оплаты ПЕРЕД выдачей */}
              {selectedOrder.status === 'ACCEPTED' && (
                <div className="bg-indigo-50/50 p-5 rounded-[32px] border border-indigo-100 space-y-3">
                  <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest text-center">Способ оплаты при выдаче</p>
                  <div className="grid grid-cols-2 gap-2">
                    <button 
                      onClick={() => setSelectedOrder({...selectedOrder, paymentMethod: 'CASH'})}
                      className={`py-3 rounded-2xl font-black text-[10px] uppercase transition-all border-2 ${selectedOrder.paymentMethod === 'CASH' ? 'bg-emerald-500 border-emerald-500 text-white shadow-md' : 'bg-white border-slate-100 text-slate-400'}`}
                    >
                      <i className="fas fa-wallet mr-2"></i>Оплачено
                    </button>
                    <button 
                      onClick={() => setSelectedOrder({...selectedOrder, paymentMethod: 'DEBT'})}
                      className={`py-3 rounded-2xl font-black text-[10px] uppercase transition-all border-2 ${selectedOrder.paymentMethod === 'DEBT' ? 'bg-red-500 border-red-500 text-white shadow-md' : 'bg-white border-slate-100 text-slate-400'}`}
                    >
                      <i className="fas fa-hand-holding-usd mr-2"></i>В долг
                    </button>
                  </div>
                </div>
              )}
              ...
              {selectedOrder.status === 'CONFIRMED' && (
                <div className="bg-emerald-50 p-4 rounded-[32px] border border-emerald-100">
                  <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">Статус оплаты</p>
                  <p className="text-sm font-bold text-emerald-700">
                    {selectedOrder.paymentMethod === 'CASH' ? 'Оплачено (Наличные/Карта)' : 'Оформлено в долг'}
                  </p>
                </div>
              )}
            ...
            <div className="pt-4 border-t border-slate-100 space-y-4">
              ...
              <div className="flex flex-col gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (!selectedOrder.paymentMethod) {
                        alert('Пожалуйста, выберите способ оплаты (Оплачено или В долг)');
                        return;
                      }
                      onConfirmOrder(selectedOrder);
                      setSelectedOrder(null);
                    }}
                    className="w-full bg-indigo-600 text-white py-5 rounded-[24px] font-black shadow-xl shadow-indigo-100 active:scale-95 transition-all uppercase tracking-widest text-xs"
                  >
                    ВЫДАТЬ ЗАКАЗ
                  </button>
                )}
                ...
                {selectedOrder.status === 'NEW' && (
                  <button
                    ...
                    className="w-full bg-amber-500 text-white py-5 rounded-[24px] font-black shadow-lg shadow-amber-100 active:scale-95 transition-all uppercase tracking-widest text-xs"
                  >
                    ПРИНЯТЬ ЗАКАЗ
                  </button>
                )}
                ...
                {(selectedOrder.status === 'NEW' || selectedOrder.status === 'ACCEPTED') && (
                  <button
                    onClick={() => { if(confirm('Отменить этот заказ?')) { onUpdateOrder({...selectedOrder, status: 'CANCELLED'}); setSelectedOrder(null); } }}
                    className="w-full py-3 font-black text-red-400 uppercase tracking-widest text-[9px]"
                  >
                    Отменить заказ
                  </button>
                )}
              ...
              {selectedOrder.status === 'CONFIRMED' && (
                <div className="flex flex-col items-center py-4 text-emerald-500 font-black text-[10px] uppercase bg-emerald-50 rounded-[24px]">
                   <i className="fas fa-check-circle mb-1 text-2xl"></i> Заказ успешно выдан
                </div>
              )}
            </div>
          </div>
      ...
      {/* Модальное окно редактирования состава заказа */}
      {isEditing && selectedOrder && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[250] flex items-end justify-center p-0" onClick={() => setIsEditing(false)}>
          <div className="bg-white w-full max-w-lg rounded-t-[40px] p-6 max-h-[85vh] overflow-y-auto animate-slide-up flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-slate-800">Редактирование заказа</h3>
            ...
            <div className="flex-1 overflow-y-auto space-y-3 mb-6 no-scrollbar">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Состав и цены</p>
              {selectedOrder.items.map((item, idx) => {
                const prod = products.find(p => p.id === item.productId);
                return (
                  <div key={idx} className="flex flex-col gap-3 p-4 bg-slate-50 rounded-3xl border border-slate-100">
                    <div className="flex justify-between items-center">
                       <p className="font-bold text-slate-800 text-sm truncate">{prod?.name || '---'}</p>
                       <button onClick={() => { const updatedItems = selectedOrder.items.filter((_, i) => i !== idx); setSelectedOrder({ ...selectedOrder, items: updatedItems, total: recalculateTotal(updatedItems) }); }} className="text-red-300 p-2"><i className="fas fa-trash-alt text-xs"></i></button>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-[8px] font-black text-slate-400 uppercase ml-1">Кол-во</label>
                        <input type="number" min="1" step="any" value={item.quantity} onChange={e => { const newQty = Math.max(0.1, parseFloat(e.target.value) || 1); const updatedItems = [...selectedOrder.items]; updatedItems[idx] = { ...item, quantity: newQty }; setSelectedOrder({ ...selectedOrder, items: updatedItems, total: recalculateTotal(updatedItems) }); }} className="w-full p-3 bg-white border border-slate-200 rounded-xl text-xs font-black text-center" />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[8px] font-black text-slate-400 uppercase ml-1">Цена (₽)</label>
                        <input type="number" step="0.01" value={item.price} onChange={e => { const newPrice = Math.max(0, parseFloat(e.target.value) || 0); const updatedItems = [...selectedOrder.items]; updatedItems[idx] = { ...item, price: newPrice }; setSelectedOrder({ ...selectedOrder, items: updatedItems, total: recalculateTotal(updatedItems) }); }} className="w-full p-3 bg-white border border-slate-200 rounded-xl text-xs font-black text-center text-indigo-600" />
                      </div>
                    </div>
                  </div>
                );
            ...
            <div className="space-y-4">
              <div className="bg-slate-800 p-5 rounded-3xl text-white flex justify-between items-center">
                <span className="text-[10px] font-black uppercase opacity-60">Новый итог:</span>
                <span className="text-2xl font-black">{selectedOrder.total.toLocaleString()} ₽</span>
              </div>

              <div className="flex gap-2">
                <button onClick={() => setIsEditing(false)} className="flex-1 py-4 text-slate-500 font-black text-[10px] uppercase rounded-[20px] border border-slate-200">Отмена</button>
                <button onClick={() => { onUpdateOrder(selectedOrder); setIsEditing(false); }} className="flex-[2] bg-indigo-600 text-white py-4 rounded-[20px] font-black text-[10px] uppercase shadow-lg shadow-indigo-100">Сохранить</button>
              </div>
            </div>
          </div>
        </div>
