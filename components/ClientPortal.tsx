...
const ClientPortal: React.FC<ClientPortalProps> = ({ user, products, sales, orders, onAddOrder }) => {
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {filteredProducts.map(p => (
              <div key={p.id} className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between min-h-[140px] animate-fade-in group">
                <div>
                  <div className="text-[9px] text-indigo-400 uppercase font-black mb-1">{p.category}</div>
                  <div className="font-bold text-slate-800 line-clamp-2 leading-tight text-sm">{p.name}</div>
                </div>
                <div className="flex justify-between items-end mt-2">
                  ...
                  <button
                    onClick={() => addToCart(p)}
                    className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center hover:bg-indigo-600 hover:text-white transition-all shadow-sm active:scale-90"
                  >
