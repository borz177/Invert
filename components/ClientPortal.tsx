...
const ClientPortal: React.FC<ClientPortalProps> = ({ 
  ...
  const [tempName, setTempName] = useState(localStorage.getItem('inventory_guest_name') || '');
  const [tempPhone, setTempPhone] = useState(localStorage.getItem('inventory_guest_phone') || '');
  ...
  useEffect(() => {
    if (activeShopId || initialShopId) return;
    const search = async () => {
  ...
  useEffect(() => {
    ...
    const fetchShopInfo = async () => {
      ...
      try {
        // Мы используем activeShopId напрямую как user_id владельца
  ...
  const myHistory = useMemo(() => {
    ...
    const myOrders = shopData.orders.filter(x => (x.customerId === shopData.customerIdInShop || x.customerId === user.id) && (x.status === 'NEW' || x.status === 'ACCEPTED' || x.status === 'CANCELLED' || x.status === 'CONFIRMED'));
  ...
  const myStats = useMemo(() => {
    ...
    const debtSales = shopData.sales.filter(x => !x.isDeleted && x.paymentMethod === 'DEBT' && (x.customerId === shopData.customerIdInShop || x.customerId === user.id));
    const allSales = shopData.sales.filter(x => !x.isDeleted && (x.customerId === shopData.customerIdInShop || x.customerId === user.id));
    const payments = shopData.cashEntries.filter(x => x.type === 'INCOME' && x.category !== 'Продажа' && (x.customerId === shopData.customerIdInShop || x.customerId === user.id));
    ...
    return { debt: Math.max(0, totalDebt - totalPaid), totalPurchased };
  }, [shopData, user.id]);
  ...
  const handleSendOrder = () => {
    ...
    localStorage.setItem('inventory_guest_name', tempName);
    localStorage.setItem('inventory_guest_phone', tempPhone);
    const newOrder: Order = { id: Date.now().toString(), customerId: shopData.customerIdInShop || user.id, items: cart.map(i => ({ productId: i.productId, quantity: i.quantity, price: i.price })), total: cartTotal, status: 'NEW', date: new Date().toISOString(), note: `${shopData.customerIdInShop ? '' : `[Имя: ${tempName}, Тел: ${tempPhone}] `}${note.trim()}` };
      ...
      <div className="space-y-6 animate-fade-in pb-32">
        ...
        {tab === 'PRODUCTS' ? (
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {filteredProducts.map(p => (
                  <div key={p.id} className="bg-white p-5 rounded-[40px] shadow-sm border border-slate-50 flex flex-col relative">
                    ...
                    <div className="space-y-1 mb-4"><p className="font-bold text-slate-800 text-sm leading-tight line-clamp-2">{p.name}</p><p className="text-[10px] text-slate-400 font-bold">Ост: {p.quantity} {p.unit}</p></div>
                    <div className="flex justify-between items-center mt-auto"><span className="font-black text-slate-800 text-lg">{p.price.toLocaleString()} ₽</span><button type="button" onClick={() => { const ex = cart.find(i => i.productId === p.id); if (ex) setCart(cart.map(i => i.productId === p.id ? {...i, quantity: i.quantity + 1} : i)); else setCart([...cart, { productId: p.id, name: p.name, price: p.price, quantity: 1, unit: p.unit }]); }} className="w-10 h-10 bg-indigo-600 text-white rounded-2xl flex items-center justify-center shadow-lg active:scale-95"><i className="fas fa-plus text-xs"></i></button></div>
                  </div>
                ))}
                {filteredProducts.length === 0 && <div className="col-span-2 text-center py-10 text-slate-300 italic">Ничего не найдено</div>}
              </div>
            ) : <div className="p-20 text-center text-slate-300 font-bold">Витрина временно недоступна</div>}
            ...
            {cart.length > 0 && (
              <button type="button" onClick={() => setIsOrdering(true)} className="fixed bottom-24 left-4 right-4 bg-indigo-600 text-white p-6 rounded-[32px] flex justify-between items-center shadow-2xl z-50 animate-slide-up">
                <div className="flex items-center gap-4"><div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center font-black">{cart.length}</div><div className="text-left"><p className="text-[10px] opacity-60 uppercase font-black">Сумма заказа</p><p className="text-lg font-black">{cartTotal.toLocaleString()} ₽</p></div></div>
          ...
          <div className="space-y-6 animate-fade-in">
            {myHistory.length > 0 || myStats.debt > 0 ? (
              <>
                <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden divide-y divide-slate-50">
                  <p className="p-5 text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50/50">Ваша история</p>
                  {myHistory.map((op: any) => (
                    <div key={op.id} onClick={() => setSelectedOpDetail(op)} className="p-5 flex justify-between items-center active:bg-slate-50 cursor-pointer">
                      <div className="min-w-0 flex-1 pr-4">
                        <div className="font-bold text-slate-800 text-sm flex items-center gap-2">{op.type === 'SALE' ? `Покупка №${op.id.slice(-4)}` : op.type === 'PAYMENT' ? 'Оплата' : 'Заявка'}</div>
                        <p className="text-[9px] text-slate-400 font-bold uppercase">{new Date(op.date).toLocaleDateString()}</p>
                        {op.type === 'ORDER' && <span className={`text-[8px] font-black uppercase inline-block px-1.5 py-0.5 rounded mt-1 ${op.status === 'NEW' ? 'bg-indigo-50 text-indigo-500' : op.status === 'ACCEPTED' ? 'bg-amber-50 text-amber-500' : op.status === 'CONFIRMED' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-400'}`}>{op.status === 'NEW' ? 'В обработке' : op.status === 'ACCEPTED' ? 'Принята' : op.status === 'CONFIRMED' ? 'Выдан' : 'Отменена'}</span>}
                      </div>
            ...
            ) : <div className="py-20 text-center space-y-4"><i className="fas fa-history text-4xl text-slate-100"></i><p className="text-slate-400 text-sm font-medium">Здесь появится ваша история</p></div>}
          </div>
        )}
        ...
        {isOrdering && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-end justify-center">
            <div className="bg-white w-full max-w-lg rounded-t-[40px] shadow-2xl p-8 flex flex-col animate-slide-up max-h-[95vh] overflow-y-auto no-scrollbar" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-6"><div><h3 className="text-xl font-black text-slate-800">Оформление</h3><p className="text-xs text-slate-400 font-bold">Укажите данные для связи</p></div><button type="button" onClick={() => setIsOrdering(false)} className="w-12 h-12 bg-slate-50 text-slate-400 rounded-full"><i className="fas fa-times"></i></button></div>
              ...
              <textarea placeholder="Пожелания к заказу..." className="w-full p-5 bg-slate-50 border border-slate-100 rounded-[24px] text-sm outline-none mb-6 resize-none" rows={2} value={note} onChange={e=>setNote(e.target.value)} />
              ...
              <button type="button" onClick={handleSendOrder} className="w-full bg-indigo-600 text-white p-6 rounded-[28px] font-black uppercase shadow-xl tracking-widest active:scale-95">ОТПРАВИТЬ МЕНЕДЖЕРУ</button>
            </div>
          </div>
        )}
        ...
        {selectedOpDetail && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[250] flex items-end justify-center p-0" onClick={() => setSelectedOpDetail(null)}>
            <div className="bg-white w-full max-w-lg rounded-t-[40px] shadow-2xl p-8 flex flex-col animate-slide-up max-h-[85vh] overflow-y-auto no-scrollbar" onClick={e => e.stopPropagation()}>
               <div className="flex justify-between items-center mb-6"><div><h3 className="text-xl font-black text-slate-800">Детали</h3><p className="text-[10px] text-slate-400 font-black uppercase">№ {selectedOpDetail.id.slice(-6)} • {new Date(selectedOpDetail.date).toLocaleDateString()}</p></div><button type="button" onClick={() => setSelectedOpDetail(null)} className="w-12 h-12 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center"><i className="fas fa-times"></i></button></div>
               {(selectedOpDetail.type === 'SALE' || selectedOpDetail.type === 'ORDER') ? (
                 <div className="space-y-4">
                    ...
                    <div className="bg-slate-800 p-6 rounded-[32px] text-white flex justify-between items-center mt-4"><span className="text-[10px] font-black uppercase opacity-60">Всего:</span><span className="text-2xl font-black">{selectedOpDetail.total.toLocaleString()} ₽</span></div>
                 </div>
               ) : <div className="bg-emerald-50 p-8 rounded-[40px] border border-emerald-100 text-center"><p className="text-[10px] font-black text-emerald-400 uppercase mb-2">Сумма оплаты</p><p className="text-4xl font-black text-emerald-600">{selectedOpDetail.amount.toLocaleString()} ₽</p></div>}
        ...
        {!initialShopId && <button onClick={() => setActiveShopId(null)} className="w-full py-4 text-[10px] font-black uppercase tracking-widest text-indigo-500">Вернуться в мои магазины</button>}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      <div className="space-y-4">
        ...
        <div className="relative"><i className="fas fa-search absolute left-6 top-1/2 -translate-y-1/2 text-slate-300"></i><input type="text" className="w-full p-6 pl-14 bg-white rounded-[32px] shadow-sm border border-slate-100 outline-none text-sm placeholder:text-slate-300" placeholder="Название..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} /></div>
        {isSearching && <div className="text-center py-6 font-bold text-indigo-400 animate-pulse">Поиск...</div>}
        <div className="space-y-3">{searchResults.map(shop => (<div key={shop.id} className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex justify-between items-center transition-all"><div className="flex items-center gap-5"><div className="w-14 h-14 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center text-2xl shadow-inner"><i className="fas fa-store"></i></div><div><p className="font-black text-slate-800">{shop.shopName}</p><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Владелец: {shop.ownerName}</p></div></div><button type="button" onClick={() => addShop(shop)} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase shadow-lg active:scale-95 transition-all">Добавить</button></div>))}</div>
      </div>
      <div className="space-y-4">
        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest px-2">Ваши магазины</h3>
        <div className="grid grid-cols-1 gap-4 overflow-hidden">{shopList.map(shop => (<div key={shop.id} className="relative overflow-hidden rounded-[40px] bg-red-500" onTouchStart={(e) => handleTouchStart(e, shop.id)} onTouchMove={(e) => handleTouchMove(e, shop.id)}><button type="button" onClick={() => removeShop(shop.id)} className="absolute right-0 top-0 bottom-0 w-24 bg-red-500 text-white font-black uppercase text-[10px] flex items-center justify-center">Удалить</button><button type="button" onClick={() => swipeId === shop.id ? setSwipeId(null) : setActiveShopId(shop.id)} style={{ transform: swipeId === shop.id ? 'translateX(-96px)' : 'translateX(0)' }} className="relative w-full bg-white p-7 shadow-sm border border-slate-50 flex items-center gap-6 transition-transform duration-300 text-left active:bg-slate-50"><div className="w-16 h-16 bg-indigo-600 text-white rounded-[24px] flex items-center justify-center text-3xl shadow-lg"><i className="fas fa-store"></i></div><div className="flex-1"><p className="text-lg font-black text-slate-800 leading-tight">{shop.shopName}</p><p className="text-xs text-slate-400 font-bold uppercase mt-1">Свайп для удаления</p></div><div className="w-10 h-10 bg-slate-50 rounded-full flex items-center justify-center text-slate-300"><i className="fas fa-chevron-right text-xs"></i></div></button></div>))}
