...
const App: React.FC = () => {
  ...
  // Функция для обновления или создания клиента при продаже
  const handleEnsureCustomerDebt = (customerId: string, amount: number, note?: string) => {
    setCustomers(prev => {
      const exists = prev.find(c => c.id === customerId);
      if (exists) {
        return prev.map(c => c.id === customerId ? { ...c, debt: (Number(c.debt) || 0) + amount } : c);
      } else {
        // Парсим имя и телефон если это заказ из портала
        let name = 'Новый клиент';
        let phone = '';
        if (note) {
          const nMatch = note.match(/\[Имя:\s*([^,]+)/);
          const pMatch = note.match(/Тел:\s*([^\]]+)/);
          if (nMatch) name = nMatch[1].trim();
          if (pMatch) phone = pMatch[1].trim();
        }
        return [...prev, { id: customerId, name, phone, debt: amount, discount: 0 }];
      }
    });
  };
  ...
  const handleConfirmOrder = (order: Order) => {
    ...
    handleEnsureCustomerDebt(order.customerId, order.total, order.note);
    setSales([newSale, ...sales]);
    setOrders(orders.map(o => o.id === order.id ? { ...o, status: 'CONFIRMED' } : o));
    alert('Заказ выдан! Данные клиента и долг сохранены.');
  };
  ...
  const handleUpdateSale = (updatedSale: Sale) => {
    ...
    oldSale.items.forEach(oldItem => {
      ...
      if (prod && prod.type !== 'SERVICE') {
        updatedProducts = updatedProducts.map(p => p.id === oldItem.productId ? { ...p, quantity: p.quantity + oldItem.quantity } : p);
      }
    });
    let updatedCustomers = [...customers];
    if (oldSale.paymentMethod === 'DEBT' && oldSale.customerId) {
      updatedCustomers = updatedCustomers.map(c => c.id === oldSale.customerId ? { ...c, debt: Math.max(0, (Number(c.debt) || 0) - oldSale.total) } : c);
    }
    updatedSale.items.forEach(newItem => {
      const prod = products.find(p => p.id === newItem.productId);
      if (prod && prod.type !== 'SERVICE') {
        updatedProducts = updatedProducts.map(p => p.id === newItem.productId ? { ...p, quantity: Math.max(0, p.quantity - newItem.quantity) } : p);
      }
    });
    if (updatedSale.paymentMethod === 'DEBT' && updatedSale.customerId) {
      const exists = updatedCustomers.find(c => c.id === updatedSale.customerId);
      if (exists) {
        updatedCustomers = updatedCustomers.map(c => c.id === updatedSale.customerId ? { ...c, debt: (Number(c.debt) || 0) + updatedSale.total } : c);
      } else {
        updatedCustomers.push({ id: updatedSale.customerId, name: 'Клиент из истории', phone: '', debt: updatedSale.total, discount: 0 });
      }
    }
  ...
  // Fix: Added missing handleClearData function to reset app state and database
  const handleClearData = async () => {
    if (!window.confirm('Вы уверены, что хотите удалить все данные? Это действие необратимо.')) return;

    setIsLoading(true);
    try {
      const keys = ['products', 'transactions', 'sales', 'cashEntries', 'suppliers', 'customers', 'employees', 'categories', 'orders', 'posCart', 'warehouseBatch'];
      await Promise.all(keys.map(key => db.saveData(key, key === 'categories' ? INITIAL_CATEGORIES : [])));
      ...
      await db.saveData('settings', DEFAULT_SETTINGS);
  ...
  const renderView = () => {
    ...
    if (isClient) return <ClientPortal user={currentUser} products={products} sales={sales} orders={orders} onAddOrder={(o) => setOrders([o, ...orders])} onActiveShopChange={setActiveClientShopName} />;
    switch (view) {
      ...
      case 'SALES': return <POS products={products} customers={customers} cart={posCart} setCart={setPosCart} currentUserId={currentUser.id} onAddCustomer={c => setCustomers([...customers, c])} onSale={s => { setSales([s, ...sales]); if (s.paymentMethod === 'DEBT' && s.customerId) handleEnsureCustomerDebt(s.customerId, s.total); else if (s.paymentMethod !== 'DEBT') handleAddCashEntry({ id: `S-${Date.now()}`, amount: s.total, type: 'INCOME', category: 'Продажа', description: `Продажа №${s.id.slice(-4)}`, date: s.date, employeeId: s.employeeId }); }}/>;
      ...
      case 'MORE_MENU': return (
        <div className="space-y-4 animate-fade-in pb-10">
          ...
          <div className="grid grid-cols-1 gap-3">
            <button onClick={() => setView('ORDERS_MANAGER')} className="w-full bg-white p-5 rounded-3xl shadow-sm flex items-center gap-4 border border-slate-100 hover:bg-slate-50"><div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center"><i className="fas fa-clipboard-list"></i></div><span className="font-bold text-slate-700">Заказы</span></button>
    ...
    <div className={`flex flex-col h-screen overflow-hidden ${settings.darkMode ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'}`}>
      <header className="bg-white text-indigo-600 p-4 shadow-sm border-b border-slate-100 flex justify-between items-center z-20 shrink-0 h-16">
        <h1 className="text-xl font-black flex items-center gap-2 cursor-pointer" onClick={() => setView(isClient ? 'CLIENT_PORTAL' : 'DASHBOARD')}>
          ...
          <span className="text-indigo-900">{isClient ? (activeClientShopName || "Инвентарь") : settings.shopName}</span>
        </h1>
