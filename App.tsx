...
const App: React.FC = () => {
  ...
  const handleUpdateSale = (updatedSale: Sale) => {
    ...
    // Если оплата была не в долг, обновляем кассу
    if (updatedSale.paymentMethod !== 'DEBT') {
      const diff = updatedSale.total - oldSale.total;
      if (diff !== 0) {
        handleAddCashEntry({
          id: `CORR-SALE-${Date.now()}`,
          amount: Math.abs(diff),
          type: diff > 0 ? 'INCOME' : 'EXPENSE',
          category: 'Корректировка чека',
          description: `Изменение суммы чека №${updatedSale.id.slice(-4)}`,
  ...
  const handleDeleteSale = (id: string) => {
    ...
    alert('Продажа удалена, товары и сумма возвращены.');
  };
  ...
  const handleClearData = async () => {
    if (!confirm('Вы действительно хотите ОЧИСТИТЬ ВСЕ ДАННЫЕ? Это действие необратимо.')) return;
  ...
  return (
    <div className={`flex flex-col h-screen overflow-hidden ${settings.darkMode ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 p-2 flex justify-around z-[70] h-20 shadow-lg">
        {isClient ? (
          <>
            <button onClick={() => setView('CLIENT_PORTAL')} className={`flex flex-col items-center justify-center w-1/2 ${view === 'CLIENT_PORTAL' ? 'text-indigo-600' : 'text-slate-400'}`}><i className="fas fa-shopping-bag text-xl mb-1"></i><span className="text-[9px] font-black uppercase">Магазин</span></button>
            <button onClick={() => setView('PROFILE')} className={`flex flex-col items-center justify-center w-1/2 ${view === 'PROFILE' ? 'text-indigo-600' : 'text-slate-400'}`}><i className="fas fa-user text-xl mb-1"></i><span className="text-[9px] font-black uppercase">Профиль</span></button>
          </>
        ) : (
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-end justify-center pb-24 px-4" onClick={() => setQuickMenuOpen(false)}>
          <div className="grid grid-cols-2 gap-4 p-6 bg-white rounded-[40px] w-full max-sm shadow-2xl animate-slide-up" onClick={e => e.stopPropagation()}>
            {QUICK_ACTIONS.map(action => (
              <button key={action.id} onClick={() => { setView(action.id as AppView); setQuickMenuOpen(false); }} className="flex flex-col items-center justify-center p-4 rounded-3xl active:bg-slate-50"><div className={`${action.color} text-white w-14 h-14 flex items-center justify-center rounded-2xl shadow-lg mb-2`}><i className={`fas ${action.icon} text-xl`}></i></div><span className="text-[10px] font-black text-slate-700 uppercase">{action.label}</span></button>
            ))}
          </div>
        </div>
      )}
